...
void DrawBoard(int n) {
	...
	GotoXY(_LEFT, _TOP); printf("%c%c%c%c", 218, 196, 196, 196);
	for (int i = 0; i < n - 1; i++)
		printf("%c%c%c%c", 194, 196, 196, 196);
	GotoXY(_LEFT + n * 4, _TOP); printf("%c", 191);
	...
	for (int i = 0; i < n - 1; i++) { GotoXY(_LEFT, 2 + _TOP + 2 * i); printf("%c%c%c%c", 195, 196, 196, 196); for (int j = 0; j < n - 1; j++) printf("%c%c%c%c", 197, 196, 196, 196); printf("%c", 180); }
...
class _Board { // tham khảo khóa trước nguồn https://www.youtube.com/watch?v=K4DFxIVM97Q
private:
	_Point** _ptrA;
public:
	int getCheckAt(int, int);
	int getXAt(int i, int j) {
		return _ptrA[i][j].GetX();
	}
	int getYAt(int i, int j) {
		return _ptrA[i][j].GetY();
	}
	void setCheckAt(int i, int j, int value) {
		_ptrA[i][j].setCheck(value);
	}
};

int _Board::getCheckAt(int i, int j) {
	return _ptrA[i][j].GetCheck();
}
